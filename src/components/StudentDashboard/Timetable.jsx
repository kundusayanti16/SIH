...
export default function Timetable() {
  ...
  const daysConfig = [
    { key: "Monday", label: t('dayMonday') || "Monday" },
    { key: "Tuesday", label: t('dayTuesday') || "Tuesday" },
    { key: "Wednesday", label: t('dayWednesday') || "Wednesday" },
    { key: "Thursday", label: t('dayThursday') || "Thursday" },
    { key: "Friday", label: t('dayFriday') || "Friday" }
  ];
  ...
  const slots = [
    ...
    { time: "12:30 PM - 01:30 PM", isBreak: true, label: `🍱 ${t('lunchBreak') || 'Lunch & Campus Recess'}` },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E7F2EB] text-[#397257] border border-[#CFE4D7] text-xs font-bold uppercase mb-2">
            <CalendarDays className="w-3.5 h-3.5" /> {t('sideTimetable') || 'Official Class Routine'}
          </div>
          <h1 className="text-2xl font-extrabold text-[#24332C] tracking-tight">
            {t('timetableTitle') || 'Weekly Timetable'} ({localizeGrade(currentUser.grade)} - {currentUser.section})
          </h1>
          <p className="text-xs text-[#718078]">{t('timetableSubtitle') || 'Class periods scheduled for your grade'}</p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs bg-white px-3 py-1.5 rounded-xl border border-[#E2E8DE] shadow-sm">
          ...
          <span className="text-[#24332C] font-semibold">{t('activeSessionBadge') || 'Mon - Fri'}</span>
        </div>
      </div>
          ...
          if (slot.isBreak) {
            return (
              <div key={idx} className="p-3 rounded-2xl bg-[#E7F2EB]/50 border border-dashed border-[#CFE4D7] text-center text-xs font-bold text-[#397257]">
                {localizeText(slot.time)} • {slot.label}
              </div>
            );
          }
            ...
            >
              <div className="flex items-start sm:items-center gap-4">
                <div className="p-3 rounded-xl bg-[#E7F2EB] text-[#397257] border border-[#CFE4D7] shrink-0 font-mono text-xs font-bold text-center min-w-[130px]">
                  ...
                  <span>{localizeText(slot.time)}</span>
                </div>

                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-extrabold text-sm text-[#24332C]">{localizeSubject(slot.subject)}</h3>
                    {slot.isClassTeacher && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#FEF8E8] text-[#976C09] border border-[#F8DC8E] font-bold flex items-center gap-1 shadow-sm">
                        <Star className="w-3 h-3 fill-[#F4C95D]" /> {t('classTeacherNoticeBadge') || 'Class Teacher'}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-3 text-xs text-[#718078] mt-1">
                    <span>{t('teacherLabel') || 'Teacher'}: <strong className="text-[#24332C]">{slot.teacher}</strong></span>
                    <span>•</span>
                    <span className="flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-[#5F9F7A]" />
                      {t('roomLabel') || 'Room'} {localizeText(slot.room)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 self-start md:self-auto">
                {daysConfig.map(d => {
                  const isActiveDay = slot.days?.includes(d.key);
                  return (
                    <span
                      key={d.key}
