...
export default function TeacherClassManager({ onOpenCreateClass }) {
  ...
  const { t, localizeText, localizeSubject, localizeGrade } = useLanguage();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E7F2EB] text-[#397257] border border-[#CFE4D7] text-xs font-bold uppercase mb-2">
            <Layers className="w-3.5 h-3.5" /> {t('classroomCommandCenter')}
          </div>
          <h1 className="text-2xl font-extrabold text-[#24332C] tracking-tight">
            {t('allManagedClassrooms')}
          </h1>
          <p className="text-xs text-[#718078]">{t('allManagedDesc')}</p>
        </div>
        ...
        >
          ...
          <span>{t('createAnotherClassroom')}</span>
        </button>
      </div>
            ...
            <div className="p-4 space-y-3.5 flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="mt-3 pt-2.5 border-t border-[#E2E8DE] flex items-center justify-between text-xs text-[#718078]">
                  <span>{t('code')}: <code className="font-mono text-[#397257] font-bold bg-[#E7F2EB] px-1.5 py-0.5 rounded border border-[#CFE4D7]">{c.code}</code></span>
